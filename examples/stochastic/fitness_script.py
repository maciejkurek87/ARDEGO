...
always_valid = [1.,10., 96.]    
maxvalue = 50.0
minVal = 50.0
maxVal = 160.0
worst_value = 50.0
optVal = 155.0 
designSpace.append( {"min":1.0,"max":32.0,"step":1.0,"type":"discrete","smin":-5.0,"smax":5.0, "set":"h"}) ## 
designSpace.append({"min":10.0,"max":40.0,"step":1.0,"type":"discrete","smin":-5.0,"smax":5.0, "set":"h"}) ## Mw
designSpace.append({"min":96.0,"max":4096.0,"step":60.0,"type":"discrete","smin":-2.0,"smax":2.0, "set":"s"}) ## Np
...
error_labels = {0:'Valid',1:'Overmap',2:'fucknows' }
...
def name():
    return "stochastic_" + platform + "_" + str(maxError) + "_doCores" + str(doCores) + "_doDf" + str(doDf) + "_errorCorrection" + str(errorCorrection)
...
def my_interpolate():
    ...
    folder_path = "/homes/mk306/all_results_stochastic_100.txt"
    ...
    for row in spamReader:
        if len(row) == 7:
            if not (float(row[6]) == 0.0 or float(row[5]) == 0.0 or float(row[5]) > 12000.0):
                try:
                    execution_time[0][(int(row[3])-10)/2][(int(row[4])-96)/60] = float(row[5]) + execution_time[0][(int(row[3])-10)/2][(int(row[4])-96)/60]
                    accuracy[0][(int(row[3])-10)/2][(int(row[4])-96)/60] = float(row[6]) + accuracy[0][(int(row[3])-10)/2][(int(row[4])-96)/60]
                    count[0][(int(row[3])-10)/2][(int(row[4])-96)/60] = count[0][(int(row[3])-10)/2][(int(row[4])-96)/60] + 1
                except:
                    pdb.set_trace()
    ...
    result_files=['all_results_stochastic_errors.txt']
