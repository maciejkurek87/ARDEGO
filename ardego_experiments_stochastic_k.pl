...
$run_dir = "examples/stochastic/"; 
$config_file = $run_dir."configuration_script.py";
$fitness_file = $run_dir."fitness_script.py";
$sampling_fancy = "True";

$results_folder = "/data/mk306/join/ardego_fx_stochastic_k_small_sample_software_latin_lambda_${limit_lambda_search}_${n_sims}_${weights_on}_${sampling_fancy}";
...
chomp($epoch);
$tempfile = "/data/mk306/temp_ardeg_stochastic_$epoch.txt";
...
#    modify($max_error_tag, $_, $fitness_file);
    ...
    foreach (@p) {
        if ($j % $parallelism == ($i - 1)){ #easiest way to divide tasks
            $old_run_dir = 'old_run_dir = "/data/mk306/join/ardego_fx_robot_small_samples_software_latin_lambda_False_5000_True_True/tT_AARDEGO_corr_anisotropic_nsims_5000_parall_2_fF_robot"';
