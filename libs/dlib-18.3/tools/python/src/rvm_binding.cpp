...
template <
    ...
    >
double _cross_validate_trainer (const trainer_type& trainer, const sample_vector& x, const label_vector& y,
        ...
        binary_test result = cross_validate_trainer(trainer, x, y, folds);
        double output = result.class1_accuracy * result.class2_accuracy;
        //outputs.push_back(result.class1_accuracy);
        //outputs.push_back(result.class2_accuracy);
        return output;
    }
...
class_<rvm_trainer<K> >* register_trainer(const char* suffix) {
	...
	def("cross_validate_trainer",
		_cross_validate_trainer<trainer_type>);
