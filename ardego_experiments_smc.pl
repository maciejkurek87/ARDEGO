...
$fitnes_function = "robot";
$run_dir = "examples/${fitnes_function}/"; 
$config_file = $run_dir."configuration_script.py";
$fitness_file = $run_dir."fitness_script.py";
$results_folder = "/data/mk306/ardego_${fitnes_function}_software_latin_lambdaLimit_${limit_lambda_search}_${n_sims}_${weights_on}_${sampling_fancy}";
...
foreach (@tuple1 = splice(@myErrors,0,2); @tuple1; @tuple1 = splice(@myErrors,0,2)) {
    ...
    $max_error_tag  = 'accuracy_limit ='; 
    @maxError = ('accuracy_limit = 4.7'); 
    foreach (@maxError) {
        $error = $_;
        #print $error;
        $p_tag  = "parall =";
