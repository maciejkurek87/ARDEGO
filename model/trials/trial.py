...
import pickle
import re
from numpy import multiply, array, ceil, floor, maximum, minimum, mean, min, max, argmax, argmin, ones, mgrid, log
    ...
    def knowledge_aware_sample_plan(self, F):
        ...
        with open(old_run_dir + '/trial-' + str(self.get_trial_no()) + "/dump/state.csv", 'r') as csvfile:
            ...
            for row in spamreader:
                if row[2] == "":
                    inval_db.append([eval(row[0].replace('.',',')), eval(row[1])])
                else :
                    val_db.append([eval(row[0].replace('.',',')), eval(row[1]), eval(row[2])])
        valids = len(val_db)
        logging.info("State dumped.. valids:" +  str(valids) + " F: " + str(F))
        to_eval_valids = minimum(valids/2,F/2)
        if valids < F:
            logging.info("The dump does not have enough examples for the plan to be worth it")
        ...
        set_to_eval_copy = copy(set_to_eval)
        logging.info("To evaluate designs number:" + str(len(set_to_eval_copy)))
        while(len(set_to_eval) > 0):
        ...
        for idx in range(len(set_to_eval_copy)):
            if not (idx in eval_set):
                ...
                code, mu, variance, ei, p = self.surrogate_model.predict(array([part, part]))
                if code[0] != 0.0: ## plug in the old designs
                    ...
                    self.set_terminating_condition(fit) 
            ...
            if self.get_model_failed():
                ...
                for best_x in best_xs:
                    ...
                    if False: ## HACK
                        ...
                        if self.surrogate_model.contains_training_instance(best_x):
...
### check first if part is already within the training set
    ...
    def initialise(self):
        ...
        try:
            ...
            logging.info("Always valid configuration not-present, make sure that the valid design space is large enough so that at least one valid design is initially evalauted")
            self.set_at_least_one_in_valid_region(False)
        self.sample_plan(10 * len(self.configuration.design_space))
