...
from numpy import unique, asarray, bincount, array, append, sqrt, log, sort, exp, isinf, all, sum, place, delete, maximum, minimum
...
class GaussianProcessRegressor4(Regressor):
    ...
    def train_nlml(self):
        ...
        if self.transLog:
            output_scaler = preprocessing.StandardScaler(with_std=False).fit(log(self.training_fitness - self.shift_by()))
            adjusted_training_fitness = output_scaler.transform(log(self.training_fitness - self.shift_by()))
        else:
    ...
    def predict(self, z, with_EI=True, raw=False): ## TODO - broken for one element z
        ...
        ## do predictions
        try:            
            vargout = gp.predict(self.i,self.m,self.k,self.l,scaled_training_set ,adjusted_training_fitness, self.input_scaler.transform(trans_z))
        except Exception,e:
            logging.error(str(e))
            return None, None, None, None
